...
parser.add_argument('-m', '--model', default='movesim', type=str, choices=['movesim', 'trajgan'], 
                    help='Model used to generate the data. Currently supported Movesim: movesim and LSTM-TrajGAN: trajgan')
...
file_path_test = f'{data_path}/{opt.model}/test.data'
file_path_validation = f'{data_path}/{opt.model}/val.data'
file_path_synthetic = f'{data_path}/{opt.model}/{epsilon}/gene.data'
file_path_train = f'{data_path}/{opt.model}/real.data'
...
if K > len(constellation_pca):
    t = datetime.now()
    current_time = t.strftime('%H:%M:%S')
    print(f'[{current_time}]: Exceeded maximum number of neighbours, new K is: {len(constellation_pca)}', flush=True)

    K = len(constellation_pca)
...
results_pth = os.listdir(f'{data_path}/{opt.model}/{epsilon}/predictions/')
...
results_pth = [pth[0:length] for pth in results_pth]
for pth in results_pth:
    if pth == file_path:
        i += 1

t = datetime.now()
current_time = t.strftime('%H:%M:%S')
print(f'[{current_time}]: Saving file as: {data_path}/{opt.model}/{epsilon}/predictions/{file_path}_{i}.pt')
torch.save(results, f'{data_path}/{opt.model}/{epsilon}/predictions/{file_path}_{i}.pt')